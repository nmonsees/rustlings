...
impl TryFrom<[i16; 3]> for Color {
    ...
    //     // let arr = arr.map(|color| u8::try_from(color).map_err(|_| Err(IntoColorError::IntConversion)));
    //     let (Ok(red), Ok(green), Ok(blue)) = arr.map(|i| u8::try_from(i).map_err(|_| IntoColorError::IntConversion)).into() else {
    //         return Err(IntoColorError::IntConversion);
    //     };
...
impl TryFrom<&[i16]> for Color {
    ...
    //     let (Ok(red), Ok(green), Ok(blue)) = (u8::try_from(*it.next().unwrap()), u8::try_from(*it.next().unwrap()), u8::try_from(*it.next().unwrap())) else {
    //         return Err(IntoColorError::IntConversion);
    //     };
